...
function Card({ id_ev, nom, saison, episode, url, image, domain, categorie, status, changeStatusInView, handleDelete }) {
    ...
    const changeStatusCard = () => {
        setStatusCard((prev) => {
            const newStatus = prev === "not finished" ? "finished" : "not finished";
            console.log(newStatus); // Ici tu as la bonne nouvelle valeur !
            changeStatusCardInDB(id_ev, newStatus);
            changeStatusInView(id_ev, newStatus);
            return newStatus;
        });
    };
    ...
    return (
        <div
            className={` ${deleteStyle} flex flex-col gap-2 rounded-md overflow-hidden shadow-md bg-white dark:bg-gray-800`}
            id={id_ev}
        >
            <div className="relative">
                ...
                {/* Menu de la card */}
                <MenuCard
                    changeDeleteCard={changeDeleteCard}
            ...
            <div className="flex flex-col gap-2 p-2 pt-0">
                {/* Nom de la série */}
                <p
                    className="text-lg color-white truncate whitespace-nowrap overflow-hidden text-ellipsis"
                    title={nom}
                >
                    {nom}
                </p>
